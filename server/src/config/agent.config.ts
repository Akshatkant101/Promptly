...
const ApplicationSchema = z.object({
  ...
  dependencies: z
    ...
    .describe("Dependencies to install (e.g., react, react-dom)"),
});
...
function displayFileTree(files: { path: string; content: string }[], folderName: string) {
...
export async function generateApplication(
  ...
  try {
    ...
    printSystem(
      chalk.red(
        `\n❌ Error generating application: ${err instanceof Error ? err.message : String(err)}\n`
      )
    );
