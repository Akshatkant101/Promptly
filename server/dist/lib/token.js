...
export async function requiredAuth() {
    ...
    if (!token) {
        console.log(chalk.red("Not authenticated. Please run 'coremind auth login --client-id <your-client-id>' to authenticate."));
        process.exit(1);
    }
    if (await isTokenExpired(token)) {
        console.log(chalk.red("Token expired. Please run 'coremind auth login --client-id <your-client-id>' to authenticate."));
        process.exit(1);
    }
    return token;
}
